...
export const addAdvancedSellerDetails = async (req, res) => {
  try {
    ...
    const { sellerType, categories, sellsBrands, gstNumber, sourceDetails } = req.body;
    ...
    const uploadGroup = async (groupName) => {
      ...
      for (let i = 0; i < files.length; i++) {
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            const url = await uploadToCloudinary(files[i].buffer, 'seller-docs');
            urls.push(url);
            break;
          } catch (err) {
            if (attempt === 3) throw new Error(`Upload failed for ${groupName} file ${i + 1}`);
          }
        }
      }
      return urls;
    ...
    res.status(200).json({
      message: "Advanced seller details submitted successfully. Awaiting admin verification."
    });
  } catch (err) {
    res.status(500).json({
